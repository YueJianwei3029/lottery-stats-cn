...
from app.services.pos_stats import position_stats, hot_cold_stats, period_list_stats, ratio_stats
from app.services.advanced_stats import std_dev_stats, percentile_stats

router = APIRouter(prefix="/api/lottery_pl3", tags=["排列3"])
TABLE = "lottery_pl3"
FIELDS = ["num_1", "num_2", "num_3"]
...
def ext_position(date: str = Query(default=None), end_date: str = Query(default=None)):
    return {"code": 200, "message": "success", "data": position_stats(TABLE, FIELDS, date=date, end_date=end_date)}


@router.get("/stats/hot_cold")
def ext_hot_cold(date: str = Query(default=None), end_date: str = Query(default=None)):
    return {"code": 200, "message": "success", "data": hot_cold_stats(TABLE, FIELDS, date=date, end_date=end_date)}


@router.get("/stats/period_list")
def ext_period_list(date: str = Query(default=None), end_date: str = Query(default=None)):
    return {"code": 200, "message": "success", "data": period_list_stats(TABLE, FIELDS, date=date, end_date=end_date)}


@router.get("/stats/ratio")
def ext_ratio(date: str = Query(default=None), end_date: str = Query(default=None)):
    return {"code": 200, "message": "success", "data": ratio_stats(TABLE, FIELDS, date=date, end_date=end_date)}
