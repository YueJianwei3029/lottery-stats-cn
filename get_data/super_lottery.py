        ...
        for line in lines:
            ...
            if len(parts) >= 29:
                data = {
                    "issue": parts[0],
                    "draw_date": parts[1],
                    "front1": clean_num(parts[2]),
                    "front2": clean_num(parts[3]),
                    "front3": clean_num(parts[4]),
                    "front4": clean_num(parts[5]),
                    "front5": clean_num(parts[6]),
                    "back1": clean_num(parts[7]),
                    "back2": clean_num(parts[8]),
                    ...
                    "prize7_count": clean_num(parts[23]),
                    "prize7_amount": clean_num(parts[24]),
                    "prize8_count": clean_num(parts[25]),
                    "prize8_amount": clean_num(parts[26]),
                    "prize9_count": clean_num(parts[27]),
                    "prize9_amount": clean_num(parts[28])
                }
                l.append(SuperLottery(**data))

        print("✅ 数据处理完成，准备入库...")

        # 重建表
        Base.metadata.drop_all(bind=engine, tables=[SuperLottery.__table__])
        Base.metadata.create_all(bind=engine, tables=[SuperLottery.__table__])
